        ...
        >
          <select
            ...
            className={`block ${
              props.value && props.value.length == 0
                ? "text-inputBorder"
                : "text-black"
            } rounded-[6px] px-2.5 pb-2.5 pt-3 w-full  h-[45px] text-txt text-gray-900  ${
              props.disabled ? "bg-lightgray" : "bg-transparent"
            }  border ${
          ...
          >
            <option className="" value="" disabled selected>
              <div className="text-gray-500">{props.placeholder}</div>
            </option>
            {genderarr.map((e, key) => {
              return (
          ...
          <FaChevronDown
            className={`absolute cursor-pointer pointer-events-none top-4 right-3 ${
