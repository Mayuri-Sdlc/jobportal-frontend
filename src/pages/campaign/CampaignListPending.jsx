...
import { getAllPendingCampaigns } from '../../features/campaign/campaignSlice'
...
const CampaignListPending = () => {
    ...
    const { pendingCampaigns: campaigns, isLoading, isError, isSuccess, message } = useSelector((state) => state.campaign)
    ...
    const handleApproveCampaign = async (status) => {
        try {
            setLoading(true)
            let payload = {
                "isVerified": status
            }
            const response = await apiPUT(`/campaign/updateStatus/${confirmationType.id}`, payload)
            if (response.data.status === 200 && response.data.data) {
                toast.success(`Campaign ${confirmationType.type} Successful!`)
                setShowConfirmationModal(false)
                dispatch(getAllPendingCampaigns(optionsData))
            }
        } catch (error) {
            console.log('handleApproveCampaign error', error);
        } finally {
            setLoading(false)
        }
    }
        ...
        <div className='flex h-screen overflow-hidden'>
                ...
                <div className='px-2 sm:px-4'>
                    <div className="overflow-x-auto w-full">
                        <div className='flex flex-wrap items-center justify-between'>
                            <div className='pt-4'>
                                <h3 className='font-bold text-xl text-secondary'>Pending Campaigns <span className='text-xl font-bold text-secondary-light'>({campaigns && campaigns?.totalResults ? campaigns.totalResults : 0})</span></h3>
                            </div>
                            <div className='pt-4'>
                                <div className="text-sm font-normal breadcrumbs">
                                    <ul>
                                        <li><Link to='/dashboard' className='text-primary'>Dashboard</Link></li>
                                        <li className='text-secondary'>Campaigns</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col">
                                        ...
                                        <table className="divide-y border-secondary-light table-auto min-w-full">
                                            <thead className="border-secondary-light border-b">
                                                <tr>
                                                    ...
                                                    <th scope="col" className="px-6 py-3 text-lg font-bold text-left text-secondary capitalize">Email</th>
                                                    <th scope="col" className="px-6 py-3 text-lg font-bold text-left text-secondary capitalize">Status</th>
                                                    <th scope="col" className="px-6 py-3 text-lg font-bold text-left text-secondary capitalize">Link</th>
                                                    <th scope="col" className="px-6 py-3 text-lg font-bold text-left text-secondary capitalize">Stage</th>
                                                ...
                                                {campaigns && campaigns.results && campaigns.results.length ?
                                                    campaigns.results.map((item) => (
                                                        <tr key={item.id}>
                                                            ...
                                                            <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap capitalize text-left"><Link to={`/campaign/${item.id}`}>{(!item.campaignName) || (item.campaignName.trim() === '' ? '--' : item.campaignName)}</Link></td>
                                                            ...
                                                            <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap capitalize text-left">{item.fundingStage}</td>
                                                            ...
                                                            <td className="flex gap-4 px-6 py-4 text-sm font-medium text-right whitespace-nowrap items-center">
                                                                <div className="cursor-pointer" onClick={() => { openModal(); setConfirmationType({ id: item.id, type: "approve" }); }}>
                                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"> <path fillRule="evenodd" clipRule="evenodd" d="M12 21.6C17.302 21.6 21.6 17.3019 21.6 12C21.6 6.69806 17.302 2.39999 12 2.39999C6.69809 2.39999 2.40002 6.69806 2.40002 12C2.40002 17.3019 6.69809 21.6 12 21.6ZM16.4486 10.4485C16.9172 9.97989 16.9172 9.2201 16.4486 8.75147C15.9799 8.28284 15.2201 8.28284 14.7515 8.75147L10.8 12.7029L9.24855 11.1515C8.77992 10.6828 8.02013 10.6828 7.5515 11.1515C7.08287 11.6201 7.08287 12.3799 7.5515 12.8485L9.9515 15.2485C10.4201 15.7172 11.1799 15.7172 11.6486 15.2485L16.4486 10.4485Z" fill="#2FA14D" /> </svg>
                                                                </div>
                                                                <div className="cursor-pointer" onClick={() => { openModal(); setConfirmationType({ id: item.id, type: "reject" }); }}>
                                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"> <path fillRule="evenodd" clipRule="evenodd" d="M12 21.6C17.302 21.6 21.6 17.3019 21.6 12C21.6 6.69806 17.302 2.39999 12 2.39999C6.69809 2.39999 2.40002 6.69806 2.40002 12C2.40002 17.3019 6.69809 21.6 12 21.6ZM10.4486 8.75147C9.97992 8.28284 9.22013 8.28284 8.7515 8.75147C8.28287 9.2201 8.28287 9.97989 8.7515 10.4485L10.303 12L8.7515 13.5515C8.28287 14.0201 8.28287 14.7799 8.7515 15.2485C9.22013 15.7172 9.97992 15.7172 10.4486 15.2485L12 13.6971L13.5515 15.2485C14.0201 15.7172 14.7799 15.7172 15.2486 15.2485C15.7172 14.7799 15.7172 14.0201 15.2486 13.5515L13.6971 12L15.2486 10.4485C15.7172 9.97989 15.7172 9.2201 15.2486 8.75147C14.7799 8.28284 14.0201 8.28284 13.5515 8.75147L12 10.3029L10.4486 8.75147Z" fill="#E84637" /> </svg>
                                                                </div>
                                                        ...
                                                        <div className="w-full flex flex-col fixed items-center justify-center mt-20">
                                                            <p className="text-lg">There are no campaigns</p>
                                                        </div>
                                                }
                                            </tbody>
                                        </table>
                            ...
                            {campaigns && campaigns.results && campaigns.results.length ? <Pagination totalPages={campaigns.totalPages} currentPage={campaigns.currentPage} handlePageNumber={handlePageNumber} /> : null}
                        </div>
                    </div>
                </div>
            </div>
            {showConfirmationModal &&
                <ConfirmationModal
                    ...
                    handleConfirm={handleApproveCampaign}
...
export default CampaignListPending