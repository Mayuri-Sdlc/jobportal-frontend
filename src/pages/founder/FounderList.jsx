...
import { getAllFounders } from '../../features/founder/founderSlice'
...
const FounderList = () => {
    ...
    const { founders, isLoading, isError, isSuccess, message } = useSelector((state) => state.founder)
    ...
    useEffect(() => {
        dispatch(getAllFounders(optionsData))
    }, [pageNumber, limit])
                ...
                <div className='px-2 sm:px-4'>
                    <div className="overflow-x-auto w-full">
                        <div className='flex flex-wrap items-center justify-between'>
                            <div className='pt-4'>
                                <h3 className='font-bold text-xl text-secondary'>Founder <span className='text-xl font-bold text-secondary-light'>({founders && founders?.totalResults ? founders.totalResults : 0})</span></h3>
                            </div>
                            <div className='pt-4'>
                                <div className="text-sm font-normal breadcrumbs">
                                    <ul>
                                        <li><Link to='/dashboard' className='text-primary'>Dashboard</Link></li>
                                        <li className='text-secondary'>Founder</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table className="divide-y border-secondary-light table-auto min-w-full">
                                        <thead className="border-secondary-light border-b">
                                            <tr>
                                                ...
                                                <th scope="col" className="px-6 py-3 text-lg font-bold text-left text-secondary capitalize">Total Invested</th>
                                        ...
                                        <tbody className="divide-y border-secondary-light">
                                            {founders && founders.results && founders.results.length ?
                                                founders.results.map((item) => (
                                                    <tr key={item.id}>
                                                        ...
                                                        <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap capitalize text-left">
                                                            <div className='flex gap-3 items-center'>
                                                                <img className='rounded-3xl w-10 h-10' src={item?.profilePic} alt="" />
                                                                <div>
                                                                    ...
                                                                    <p className='text-sm font-normal text-[#6B788E]'>{item.companies.map(cItem => cItem?.name).join(',')}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        ...
                                                        <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap">{item.amount ? `£ ${item.amount}` : '--'}</td>
                                                        <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap capitalize text-left">{moment(item.createdDate).format('ddd MMM DD YYYY')} <br /><small className='text-md'>{moment(item.createdDate).format('HH:mm:ss Z')}</small></td>
                                                        <td className="flex gap-6 px-6 py-4 text-lg font-normal text-right whitespace-nowrap items-center">
                                                    ...
                                                    <div className="w-full flex flex-col fixed items-center justify-center mt-20">
                                                        <p className="text-lg">There are no Founders</p>
                                                    </div>
                                            }
                                        </tbody>
                                    </table>
                        ...
                        {founders && founders.results && founders.results.length ? <Pagination totalPages={founders.totalPages} currentPage={founders.currentPage} handlePageNumber={handlePageNumber} /> : null}
                    </div>
                </div>
            </div>
        </div>
