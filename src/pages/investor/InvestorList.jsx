...
const InvestorList = () => {
    ...
    const { investors, isLoading, isError, isSuccess, message } = useSelector((state) => state.investor)
                ...
                <div className='px-2 sm:px-4'>
                    <div className="overflow-x-auto w-full">
                        <div className='flex flex-wrap items-center justify-between'>
                            <div className='pt-4'>
                                <h3 className='font-bold text-xl text-secondary'>Investor <span className='text-xl font-bold text-secondary-light'>({investors && investors?.totalResults ? investors.totalResults : 0})</span></h3>
                            </div>
                            <div className='pt-4'>
                                <div className="text-sm font-normal breadcrumbs">
                                    <ul>
                                        <li><Link to='/dashboard' className='text-primary'>Dashboard</Link></li>
                                        <li className='text-secondary'>Investor</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="inline-block min-w-full overflow-hidden">
                                    <table className="divide-y border-secondary-light table-auto min-w-full">
                                        ...
                                        <tbody className="divide-y border-secondary-light">
                                            {investors && investors.results && investors.results.length ?
                                                investors.results.map((item) => (
                                                    <tr key={item.id}>
                                                        ...
                                                        <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap capitalize text-left">
                                                            <div className='flex gap-3 items-center'>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    {item.role.type == 'institutional' ? <p className='text-sm font-normal text-[#6B788E]'>{item.companies.map(cItem => cItem.name).join(',')}</p> : null}
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap capitalize text-left">{item.email}</td>
                                                        <td className="px-6 py-4 text-lg font-normal text-secondary whitespace-nowrap capitalize text-left">{`${item?.role?.type} Investor`}</td>
                                                    ...
                                                    <div className="w-full flex flex-col fixed items-center justify-center mt-20">
                                                        <p className="text-lg">There are no investors</p>
                                                    </div>
                                            }
                                        </tbody>
                                    </table>
                        ...
                        {investors && investors.results && investors.results.length ? <Pagination totalPages={investors.totalPages} currentPage={investors.currentPage} handlePageNumber={handlePageNumber} /> : null}
                    </div>
                </div>
            </div>
        </div>
